...
uses
   Windows, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ImgList,
   StdCtrls, ComCtrls, ExtCtrls, Registry;

type
   OptionsStringTypes = (ostFileAssociation,cstLanguage,wstAssociationError);
   ...
   private
      ...
      MyLanguage : array[OptionsStringTypes] of string;
...
procedure TFrmOptions.LoadLanguage;
begin
   //OptionsStringTypes = (ostLanguage,wstAssociationError);

   LblOptionsTitle.Caption := FrmBIGMain.Language.GetString('Options','Preferences');
   LblOptionsDescription.Caption := FrmBIGMain.Language.GetString('Options','PreferencesDescription');
   AssociateCheck.Caption := FrmBIGMain.Language.GetString('Options','AssociateBIGWithMe');
   GpbIconSelect.Caption := FrmBIGMain.Language.GetString('Options','Icon');
   // LblSelectLanguage.Caption := FrmBIGMain.Language.GetString('Options','SelectLanguage');
   BtnApply.Caption := FrmBIGMain.Language.GetString('Common','Apply');
   BtOK.Caption := FrmBIGMain.Language.GetString('Common','OK');
   BtCancel.Caption := FrmBIGMain.Language.GetString('Common','Cancel');
   MyLanguage[ostFileAssociation] := FrmBIGMain.Language.GetString('Options','FileAssociation');
   MyLanguage[cstLanguage] := FrmBIGMain.Language.GetString('Common','Language');
   MyLanguage[wstAssociationError] := FrmBIGMain.Language.GetString('Warnings','AssociationFailure');
end;
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            begin
               ShowMessage(FrmBIGMain.Language.GetString('Warnings','AssociationFailure'));
            end;
         end
         else
         begin
...
procedure TFrmOptions.Pref_ListKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   Pref_ListClick(sender);
end;

procedure TFrmOptions.Pref_ListKeyPress(Sender: TObject; var Key: Char);
begin
   Pref_ListClick(sender);
end;

procedure TFrmOptions.Pref_ListKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   Pref_ListClick(sender);
end;
